    ...
    if (typeof db !== 'undefined' && db) {
        db.collection("announcements").onSnapshot((snapshot) => {
            ...
            items.forEach((item) => {
                ...
                html += `
                    <div data-id="${id}" id="${id}" onclick="openAnnouncementDetailModal('${id}')" class="ad-card announcement-card academy-card p-6 rounded-3xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#111b21] shadow-lg hover:border-tsMavi transition-all flex flex-col justify-between space-y-4 relative overflow-hidden cursor-pointer group">
                ...
                reqContainer.innerHTML = `
                    <form onsubmit="submitJoinRequest(event, '${announcementId}')" class="space-y-3">
                        <h4 class="font-bold text-sm flex items-center gap-2">
                            <svg class="w-4 h-4 text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/></svg>
                            <span>Bu Ekibe / Gruba Katılma İsteği Gönder</span>
                        </h4>
                        <div>
